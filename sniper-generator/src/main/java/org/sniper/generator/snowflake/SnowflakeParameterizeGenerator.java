 ...
 * 2.其中当前毫秒内的序列号采用的是RadomTimeSequence实现类生成的，与默认的TimeSequence不同的是，RadomTimeSequence在重置时，
 * 不会像TimeSequence那样直接重置为0，而是重置成[0,4095]区间内任意一个随机数，这样做的目的有如下两点：</p>
 * 1)避免每个毫秒时刻内生成的第一笔序列，其末尾的序列号都是从0开始的。例如：若这些序列用来做哈希散列存储，系统在大多数情况下每毫秒就只生成了一笔，
 ...
 * 4.将第1步和第3步的结果进行拼接则就是最终的结果，例如：2142664137405042"</p>
...
public class SnowflakeParameterizeGenerator extends AbstractSnowflakeGenerator<Long>
	...
	/**
	 * 根参数生成结果，并将参数的最后2位作为序列号作为返回
	 * @author <a href="mailto:code727@gmail.com">杜斌</a> 
	 * @param parameter
	 * @return
	 */
	protected String generateByParameter(Object parameter) {
		String generatedValue = StringUtils.substringEnd(parameterGenerator.generate(parameter).toString(), 2);
