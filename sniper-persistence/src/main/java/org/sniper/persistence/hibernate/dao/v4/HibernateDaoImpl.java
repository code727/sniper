...
package org.sniper.persistence.hibernate.dao.v4;
...
import org.sniper.persistence.hibernate.HibernateUtils;
import org.sniper.persistence.hibernate.dao.HibernateCriteriaQueryCallback;
import org.sniper.persistence.hibernate.dao.HibernateCriteriaQueryCallbackDao;
...
public class HibernateDaoImpl<T, PK extends Serializable> extends
	...
	public T loadById(String entityName, PK id, LockMode lockMode) {
		return (T) (StringUtils.isNotBlank(entityName) ? 
				getCurrentSession().load(entityName.trim(), id, lockMode) :
					getCurrentSession().load(this.getTargetType(), id, lockMode));
	}
	...
	public T loadById(final String entityName, final PK id, final LockOptions lockOptions) {
		if (StringUtils.isNotBlank(entityName)) 
			return (T) (lockOptions != null ? 
					getCurrentSession().load(entityName.trim(), id, lockOptions) : 
						getCurrentSession().load(entityName.trim(), id));
		else
			return (T) (lockOptions != null ? 
					getCurrentSession().load(getTargetType(), id, lockOptions) :
						getCurrentSession().load(getTargetType(), id));
	}

	@SuppressWarnings("unchecked")
	@Override
	public List<T> loadAll() {
		Criteria criteria = getCurrentSession().createCriteria(getTargetType());
	...
	public T getById(PK id) {
		return getById(id, (LockOptions) null);
	}
	...
	public T getById(String entityName, PK id) {
		return getById(entityName, id, (LockOptions) null);
	}
	...
	public T getById(String entityName, PK id, LockMode lockMode) {
		...
		return (T) (StringUtils.isNotBlank(entityName) ? 
				getCurrentSession().get(entityName.trim(), id, lockMode) : 
				getCurrentSession().get(this.getTargetType(), id));
	}
	...
	public T getById(final String entityName, final PK id, final LockOptions lockOptions) {
		if (StringUtils.isNotBlank(entityName)) 
			return (T) (lockOptions != null ? 
					getCurrentSession().get(entityName.trim(), id, lockOptions) : 
						getCurrentSession().load(entityName.trim(), id));
		else
			return (T) (lockOptions != null ? 
					getCurrentSession().get(getTargetType(), id, lockOptions) :
						getCurrentSession().load(getTargetType(), id));
	}
	...
	public <P> List<T> findByCriteria(P parameter, final int start,
		...
		Criteria criteria = getCurrentSession().createCriteria(this.getTargetType());
		callback.execute(criteria);
