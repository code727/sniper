 ...
 * @version 1.0
 */
public abstract class AbstractRequestHandler extends AbstractEncodableHandler implements RequestHandler {

	@Override
	public String getEncoding(HttpForm form) {
		String encoding = form.getEncoding();
		// 获取表单绑定的字符集编码，否则返回当前处理器绑定的字符集编码
		return StringUtils.isNotBlank(encoding) ? encoding : super.getEncoding();
	}
